...
public class Board extends JPanel implements Cloneable {
	...
	public void move(Move move, boolean relinquish) {
		...
		if (relinquish) {
			...
			if (this.getColor() == Piece.WHITE) {
				if (this.isLocked()) {
					table.setStatus("You're white, your turn...");

					this.table.getAda().isSearching(false);

					this.isLocked(Piece.BLACK);
				} else {
					table.setStatus("Now it's Ada's turn. Thinking...");

					this.isLocked(true);

					Analysis analysis = this.table.getAda().getAnalysis();

					this.table.getAda().isSearching(true);

					new Search(this, analysis, Piece.BLACK);
				}
			} else {
				/* A human can never play as black, only Ada can. */
			}
		}
	}
	...
	protected void paintComponent(Graphics g) {
		...
		if (this.table.getAda().isSearching()) {
	...
	class Controller extends JPanel implements MouseListener, MouseMotionListener {
		...
		public void mouseReleased(MouseEvent event) {
			...
			/* Always make sure we aren't hold a piece. */
			this.piece = null;
