	...
	public static long getAttackBitmap(Piece piece, Bitboard bitboard) {
		...
		if (piece.getColor() == Piece.WHITE) {
			...
			if (Bitboard.getPositionFromBitmap(bitmap).getRank() == 6) {
				if (bitboard.getBitmapAtPosition(new Position(rank, file - 1)) != 0) {
					result = (bitmap >>> 8);
				} else {
					if (bitboard.getBitmapAtPosition(new Position(rank - 1, file - 1)) != 0) {
						result = (bitmap >>> 8);
					} else {
			...
			if (bitboard.getBitmapAtPosition(Piece.BLACK, new Position(rank, file - 1)) > 0) {
				result = 0;
			}
			...
			if (Bitboard.getPositionFromBitmap(bitmap).getRank() == 1) {
				if (bitboard.getBitmapAtPosition(new Position(rank, file - 1)) != 0) {
					...
					if (bitboard.getBitmapAtPosition(new Position(rank + 4, file - 1)) != 0) {
						result = (bitmap << 8);
					} else {
						result = (bitmap << 8) | (bitmap << 16);
					}
			...
			/* Make sure we can't advance when we're being blocked... */
			if (bitboard.getBitmapAtPosition(Piece.WHITE, new Position(rank, file + 1)) > 0) {
				result = 0;
			}
